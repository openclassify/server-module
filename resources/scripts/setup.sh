...
sudo chown www-data:pure -R /var/www/html
...
5 2 * * * cd /var/www/html/utility/pure-update && sh run.sh >> /dev/null 2>&1
EOF
crontab $TASK
sudo systemctl restart nginx.service
...
PUREBULD=/var/www/html/build_$SERVERID.php
sudo touch $PUREBULD
sudo cat > "$PUREBULD" <<EOF
$BUILD
EOF
PUREPING=/var/www/html/ping_$SERVERID.php
sudo touch $PUREPING
...
sudo touch $ARTISAN
UPDATE=/var/www/html/utility/pure-update/run.sh
sudo touch $UPDATE
